...
rows = []
for path in sorted(Path("outputs").glob("ps_a11_test_ps_a11_chb*_s42/checkpoint_test_evaluation.json")):
    data = json.loads(path.read_text(encoding="utf-8"))
    case = data["source_run_id"].replace("ps_a11_", "").replace("_s42", "")
...
if not rows:
    raise SystemExit("No A1.1 test results found.")
bals = [r[1] for r in rows]
mean = statistics.fmean(bals)
sd = statistics.stdev(bals) if len(bals) > 1 else 0.0
a10 = 0.9071853608224304
print(f"N={len(bals)} mean_balanced={100*mean:.3f}% sd={100*sd:.3f}% | A1.0_mean={100*a10:.3f}% delta={100*(mean-a10):+.3f}pp | target=95 pass={mean>=0.95}")
out = {
    "protocol": "path_a_a1_1_multiscale_residual_compact_25k",
    "n_cases": len(bals),
...
Path("outputs/ps_a11_cohort_test_summary.json").write_text(json.dumps(out, indent=2) + "\n", encoding="utf-8")
print("Wrote outputs/ps_a11_cohort_test_summary.json")
# Also write to tracked reports path if present
rep = Path("research_v2/reports/path_a")
rep.mkdir(parents=True, exist_ok=True)
(rep / "ps_a11_cohort_test_summary.json").write_text(json.dumps(out, indent=2) + "\n", encoding="utf-8")
print(f"Also wrote {rep / 'ps_a11_cohort_test_summary.json'}")
PY
