#!/usr/bin/env bash
# Sealed test eval for each trained case model (no retraining).
set -euo pipefail
...
for run_dir in outputs/ps_a1_chb*_s42; do
  [[ -d "${run_dir}" ]] || continue
  ...
  export CHBMIT_RUN_ID="ps_a1_test_${src}"
...
echo "Done. Aggregate with tools/patient_specific/05_aggregate.sh"
