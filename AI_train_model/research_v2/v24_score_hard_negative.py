"""V2.4 train-only, score-ranked hard-negative cache construction."""
...
def _v24_preprocessing(config: dict) -> dict:
    return {
...
def _hash_checked_source(source_dir: Path, source: dict) -> tuple[Path, np.ndarray, np.ndarray]:
    ...
    if model_spec.get("architecture") != "paper_a_multiscale_residual_1dcnn" or model_spec.get("parameter_count") != 57446:
        raise ValueError("V2.4 requires the exact 57,446-parameter C1 source model")
...
def _verify_v21_cache(prepared_dir: Path, manifest_path: str | Path) -> None:
    ...
    if (prepared_dir / "chbmit_test.npz").exists() or (prepared_dir / "continuous_test_recordings.csv").exists():
        raise ValueError("V2.4 refuses a source cache containing a sealed-test artifact")
    ...
    if summary.get("protocol") != "research_v2_1_confirmation" or int(summary.get("window_samples", 0)) != 1280:
        raise ValueError("V2.4 requires a verified V2.1 five-second confirmation cache")
...
def select_score_ranked_candidates(
    starts: np.ndarray,
    ...
    """Return all clean unsampled interictal windows ranked later by source score."""
    starts = np.asarray(starts, dtype=np.int64)
    probabilities = np.asarray(probabilities, dtype=np.float64)
    if len(starts) != len(probabilities):
        raise ValueError("Score timestamps and probabilities differ in length")
    candidates, source_sampled = [], 0
    for position, start in enumerate(starts):
        if int(start) not in clean_starts:
            continue
        if (recording_id, int(start)) in source_normal_keys:
            source_sampled += 1
            continue
        candidates.append({
            ...
            "source_score": float(probabilities[position]),
        })
    return candidates, {
        "clean_interictal_candidates": len(candidates),
        "excluded_source_sampled_normals": source_sampled,
    }
...
def _validate_existing_output(output_dir: Path, config: dict, manifest_path: str | Path) -> dict | None:
    summary_path = output_dir / "score_ranked_hard_negative_mining_summary.json"
    if not output_dir.exists():
        return None
    if not summary_path.is_file():
        raise RuntimeError(f"Refusing incomplete V2.4 hard-negative cache: {output_dir}")
    with summary_path.open("r", encoding="utf-8") as input_file:
        summary = json.load(input_file)
    expected = {
        "protocol": "research_v2_4_score_ranked_hard_negative",
        "protocol_hash": canonical_json_hash(config),
        "fold_manifest_sha256": file_sha256(manifest_path),
    }
    if any(summary.get(key) != value for key, value in expected.items()):
        raise RuntimeError(f"Existing V2.4 cache contract differs: {output_dir}")
    source = config["hard_negative_mining"]["source_runs"].get(str(summary.get("fold_index")))
    if source is None:
        raise RuntimeError(f"Existing V2.4 cache has an unknown fold: {output_dir}")
    if summary.get("source_checkpoint_sha256") != source["checkpoint_sha256"]:
        raise RuntimeError(f"Existing V2.4 cache used a different source checkpoint: {output_dir}")
    ...
    if frozen.get("source_mean_sha256") != source["scaler_mean_sha256"] or frozen.get("source_scale_sha256") != source["scaler_scale_sha256"]:
        raise RuntimeError(f"Existing V2.4 cache used a different source scaler: {output_dir}")
    ...
    if any(not (output_dir / name).is_file() for name in required):
        raise RuntimeError(f"Existing V2.4 cache is incomplete: {output_dir}")
    if summary.get("source_train_scores_sha256") != file_sha256(output_dir / "source_train_scores.npz"):
        raise RuntimeError(f"Existing V2.4 cache source scores differ: {output_dir}")
    if summary.get("source_train_score_records_sha256") != file_sha256(output_dir / "source_train_scores.records.json"):
        raise RuntimeError(f"Existing V2.4 cache source score metadata differs: {output_dir}")
    if (output_dir / "chbmit_test.npz").exists() or (output_dir / "continuous_test_recordings.csv").exists():
        raise RuntimeError(f"Existing V2.4 cache contains a sealed-test artifact: {output_dir}")
    return summary
...
def build_score_ranked_hard_negative_cache(
    *,
    ...
    preprocessing = _v24_preprocessing(config)
    ...
    try:
        ...
        for row_index, (row, record) in enumerate(zip(train_rows, scores["records"])):
            ...
            candidates, counts = select_score_ranked_candidates(
                ...
                row["recording_id"], row["patient_group"],
            )
        ...
        if len(selected) != requested:
            raise RuntimeError(
                f"V2.4 requires {requested} score-ranked hard negatives but only {len(selected)} are eligible"
            )
        ...
        summary = {
            ...
            "raw_clean_score_ranked_candidates": int(len(raw_candidates)),
            "separated_clean_score_ranked_candidates": int(len(separated)),
            "source_score_diagnostics": dict(aggregate),
            "selected_source_score": {
                "minimum": float(min(item["source_score"] for item in selected)),
                "median": float(np.median([item["source_score"] for item in selected])),
                "maximum": float(max(item["source_score"] for item in selected)),
            },
        ...
        save_json(output_dir / "preparation_summary.json", {
            ...
            "source_v21_cache": str(source_prepared_dir), "hard_negative_summary": str(output_dir / "score_ranked_hard_negative_mining_summary.json"),
        })
        save_json(output_dir / "score_ranked_hard_negative_mining_summary.json", summary)
        return summary
    except Exception:
