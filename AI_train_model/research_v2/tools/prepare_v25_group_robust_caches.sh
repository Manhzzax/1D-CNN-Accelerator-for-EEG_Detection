#!/usr/bin/env bash
# Verify the read-only V2.1 causal caches before V2.5 group-robust training.
set -euo pipefail
...
cd "$root"
python -m research_v2 validate --protocol research_v2/configs/protocol_v2_5.json --registry research_v2/configs/candidate_registry_v2_5.json
...
if set(summary.get("outputs", {})) != expected:
    raise SystemExit("Cache splits do not match the V2.5 development contract")
print(f"F{sys.argv[2]} cache verified: {summary['fold_manifest_sha256']}")
PY
done

echo "V2.5 cache preflight passed. Reusing read-only V2.1 causal windows; blocks 5 and 6 remain absent."
