#!/usr/bin/env bash
# Confirm one inner-selected hierarchical CNN configuration on all V2 seeds.
set -euo pipefail

if [[ $# -ne 7 ]]; then
  echo "Usage: $0 <prepared_train_val_dir> <run_prefix> <kernel_1> <kernel_2> <kernel_3> <learning_rate> <weight_decay>" >&2
  exit 2
fi
...
[[ "$run_prefix" =~ ^v2_f[0-9][0-9]_[a-z0-9_]+$ ]] || {
  echo "run_prefix must begin with v2_fNN_ and contain only lowercase letters, digits, or underscores" >&2
  exit 1
}
