#!/usr/bin/env bash
# Build one V2.3 train-only policy-aligned hard-negative cache for a development fold.
set -euo pipefail
...
cd "$root"
protocol="research_v2/configs/protocol_v2_3.json"
registry="research_v2/configs/candidate_registry_v2_3.json"
...
output="research_v2/generated_v23/f${fold}_policy_hardneg"
...
[[ ! -e "$output/chbmit_test.npz" && ! -e "$output/continuous_test_recordings.csv" ]] || {
  echo "Refusing a V2.3 cache containing sealed-test artifacts" >&2; exit 1;
}

python -m research_v2 v23-mine-policy-hard-negatives \
...
echo "V2.3 F${fold} cache is ready. Re-running this command verifies/reuses it without re-scoring train EEG."
